...
class TestPositive:
    ...
    def test_step2(self, clear_folders, make_files):
        ...
        res.append(ssh_checkout(data['host'], data['user'], data['passwd'],
                                "cd {}; 7z a {}/arx".format(data["folder_in"], data["folder_out"]), "Everything is Ok"))
        res.append(ssh_checkout(data['host'], data['user'], data['passwd'],
                                "cd {}; 7z e arx.7z -o{} -y".format(data["folder_out"], data["folder_ext"]),
                                "Everything is Ok"))
    ...
    def test_step3(self):
        # test3
        assert ssh_checkout(data['host'], data['user'], data['passwd'], "cd {}; 7z t arx.7z".format(data["folder_out"]),
                            "Everything is Ok"), "test3 FAIL"

    def test_step4(self):
        # test4
        assert ssh_checkout(data['host'], data['user'], data['passwd'], "cd {}; 7z u arx2.7z".format(data["folder_in"]),
    ...
    def test_step8(self, clear_folders, make_files):
        ...
        for i in make_files:
            res.append(
                ssh_checkout(data['host'], data['user'], data['passwd'], "cd {}; 7z h {}".format(data["folder_in"], i),
                             "Everything is Ok"))
